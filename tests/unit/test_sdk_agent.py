...
def _build_mock_transport(
...
) -> tuple[OAuthSession, CloudOAuthTransport]:
    ...
    session.refresh = MagicMock(
        return_value=TokenData(
            access_token=f"{access_token}_new",
            token_type="Bearer",
            expires_in=3600,
            refresh_token=f"{refresh_token}_new",
        )
    )
    ...
    def _mock_rpc(method, params, retryable=False):
        ...
        if method == "tools/call":
                        ...
                        "content": [
                            {
                                "type": "text",
                                "text": json.dumps(
                                    {"status": "success", "data": {"memories": matches}}
                                ),
                            }
                        ]
                    }
            ...
            if name == "memory.update":
                ...
                if (access_token == "tok_a" and target_id == "rec-b-456") or (
                    access_token == "tok_b" and target_id == "rec-123"
                ):
                    raise ProtocolError("not_found", "Cross-tenant record not found")
...
def test_controlled_memory_agent_15_steps_success() -> None:
    ...
    assert report["step_details"]["15_tenant_isolation"]["database_rls_directly_proven"] is False
    assert report["step_details"]["2_oauth_pkce_login"]["interactive_login_performed"] is False
...
def test_forged_authority_fails_when_server_does_not_reject() -> None:
    ...
    mock_resp_200.__enter__.return_value.read.return_value = json.dumps(
    ...
    assert (
        report["step_details"]["14_forged_authority_rejection"]["error_code"] == "validation_failed"
    )
...
def test_tenant_isolation_fails_without_second_tenant() -> None:
    ...
    assert report["step_details"]["15_tenant_isolation"]["error_code"] == "validation_failed"
...
def test_provenance_fails_when_missing_or_mismatching() -> None:
    ...
    assert report["step_details"]["10_provenance_preservation"]["error_code"] == "validation_failed"
...
def test_update_fails_without_version_increment() -> None:
    ...
    assert report["step_details"]["7_update"]["error_code"] == "validation_failed"
...
def test_forget_fails_without_tombstone_status() -> None:
                    ...
                    "content": [
                        {
                            ...
                            "text": json.dumps(
                                {
                                    "status": "success",
                                    "data": {"id": "rec-123", "status": "unknown"},
                                }
                            ),
                        }
                    ]
    ...
    assert report["step_details"]["8_forget"]["error_code"] == "validation_failed"
