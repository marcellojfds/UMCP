...
| `gcp-deployment.md` (superseded; see [`gcp-infrastructure.md`](../../runbooks/gcp-infrastructure.md)) | O runbook histórico declarava `ACTIVE / PRODUCTION-READY` e instruções de `curl`/deploy. | Que Cloud Run, Cloud SQL, KMS, Secret Manager ou Artifact Registry foram criados/configurados como descrito. |
...
| CI autenticava com chave JSON estática. | O workflow histórico `gcp-deploy.yml` foi substituído por [`gcp-review.yml`](../../../.github/workflows/gcp-review.yml). | **P0 blocker histórico.** Material de longa duração e sem escopo/proveniência demonstrados. |
| Cloud SQL habilitava IPv4 público e não havia VPC, Private Service Access, connector ou proxy configurado. | O histórico `cloud_sql.tf` foi substituído por [`network_sql.tf`](../../../ops/terraform/gcp/network_sql.tf). | **P0 blocker histórico.** Viola o requisito de banco privado. |
...
| Secret Manager era criado, mas Cloud Run não referenciava versões de segredo. | O histórico `secret_manager.tf` foi substituído por [`secrets_kms.tf`](../../../ops/terraform/gcp/secrets_kms.tf). | **P0 blocker histórico.** “Secrets somente no Secret Manager” não estava satisfeito. |
| KMS criava uma CryptoKey sem integração suficiente. | O histórico `kms.tf` foi substituído por [`secrets_kms.tf`](../../../ops/terraform/gcp/secrets_kms.tf); [`src/omp/config.py`](../../../src/omp/config.py) e [ADR 0013](../../adr/0013-server-decryptable-envelope-encryption.md) registram a evolução posterior. | **P0 blocker histórico de claim criptográfico.** |
| Endpoint HTTP reutilizava adapter local. | O histórico `src/omp/server/http.py` foi substituído pela composição hospedada em [`official.py`](../../../src/omp/server/official.py). | **P0 blocker histórico.** Não havia validação OIDC/OAuth, principal imutável, scopes, consentimento, revogação ou rejeição hosted comprovada. |
| Backups/PITR eram apenas flags de instância. | O histórico `cloud_sql.tf` foi substituído por [`network_sql.tf`](../../../ops/terraform/gcp/network_sql.tf). | **P0 blocker histórico de privacy/restore.** Backup configurado não é restore aceito. |
...
- [IaC GCP](../../../ops/terraform/gcp/),
  workflow histórico `gcp-deploy.yml` (substituído por [`gcp-review.yml`](../../../.github/workflows/gcp-review.yml)),
  [script de deploy](../../../scripts/deploy-gcp.sh) e
  runtime HTTP histórico (substituído por [`official.py`](../../../src/omp/server/official.py)).
