...
export function renderMemoryBrowser({ items, query = "", space = "", state = "", type = "", view = "cards" }) {
  ...
  return {
    content: `<div class="vault-filterbar"><span aria-live="polite">${count}</span>${chips}<button type="button" class="filter-chip${activeFilters ? " is-active" : ""}" data-toggle-filters aria-expanded="${activeFilters ? "true" : "false"}">Filters${activeFilters ? ` (${activeFilters})` : ""} <span aria-hidden="true">＋</span></button></div>${filters}<section class="vault-memory-grid${view === "list" ? " is-list" : ""}" aria-label="Memories">${visible.length ? visible.map(memoryCard).join("") : empty}</section>`,
    toolbar: `<div class="account-page-actions" aria-label="Memory view"><a class="icon-button${view === "cards" ? " is-active" : ""}" href="${memoryViewHash({ query, space, state, type, view: "cards" })}" aria-label="Card view"${view === "cards" ? ' aria-current="true"' : ""}>▦</a><a class="icon-button${view === "list" ? " is-active" : ""}" href="${memoryViewHash({ query, space, state, type, view: "list" })}" aria-label="List view"${view === "list" ? ' aria-current="true"' : ""}>☷</a></div>`,
  };
}
