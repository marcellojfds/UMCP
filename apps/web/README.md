...
Open `index.html` from a local static server for visual review. By default the shell is in `unavailable` mode: it explains the missing backend instead of showing invented memories or pretending an email was sent. To connect it, a server-rendered bootstrap may set `window.__UMCP_ADMIN_ADAPTER__` to an adapter implementing the methods documented in `src/admin-adapter.js`.
