...
class CloudOAuthTransport(ToolTransport):
    ...
    def discover(self) -> Mapping[str, Any]:
        ...
        return {
            ...
            "server_name": init_res.get("result", {})
            .get("serverInfo", {})
            .get("name", "umcp-cloud"),
            "server_version": init_res.get("result", {})
            .get("serverInfo", {})
            .get("version", "1.0"),
    ...
    def call_tool(self, name: str, arguments: Mapping[str, Any]) -> Mapping[str, Any]:
        ...
        is_safe = name == "memory.search" or "idempotency_key" in arguments

        res = self._rpc(
            "tools/call", {"name": name, "arguments": dict(arguments)}, retryable=is_safe
        )
                ...
                if isinstance(item, dict) and item.get("type") == "text":
                    ...
                    try:
                        ...
                        if is_error:
                            return {
                                "ok": False,
                                "error": {"code": "tool_error", "message": text_val},
                            }
            ...
            try:
                with urlopen(req, timeout=self.session.timeout) as resp:
                    ...
                    try:
                        ...
                        raise ProtocolError(
                            "invalid_response",
                            f"Could not decode JSON or SSE response: {raw[:150]}",
                        )
                ...
                if status == 401:
                    ...
                    if (
                        attempts == 1
                        and self.session._tokens
                        and self.session._tokens.refresh_token
                    ):
                        try:
                ...
                if retryable and attempts < self.max_retries:
                    ...
                    continue
                raise ProtocolError(
                    "network_error", f"Request to {self.endpoint} failed: {exc}"
                ) from exc
